...
# Límites de planes
PLAN_LIMITES = {
    "free": {"tarjetas_por_solicitud": 2, "solicitudes_por_hora": 3, "duracion_dias": 0, "precio": 0},
    "basico": {"tarjetas_por_solicitud": 5, "solicitudes_por_hora": 10, "duracion_dias": 7, "precio": 10},
    "premium": {"tarjetas_por_solicitud": 10, "solicitudes_por_hora": 20, "duracion_dias": 7, "precio": 20},
    "vip": {"tarjetas_por_solicitud": 15, "solicitudes_por_hora": 35, "duracion_dias": 7, "precio": 30}
}
...
def obtener_info_usuario_completa(user_id: int) -> dict:
    conn = sqlite3.connect(DB_USUARIOS)
    c = conn.cursor()
    c.execute("SELECT id, plan, ultima_solicitud, solicitudes_realizadas, fecha_registro, fecha_expiracion FROM usuarios WHERE id = ?", (user_id,))
    resultado = c.fetchone()
    conn.close()
    ...
    id_usuario, plan, ultima_solicitud, solicitudes_realizadas, fecha_registro, fecha_expiracion = resultado
...
def puede_realizar_solicitud(user_id: int) -> bool:
    conn = sqlite3.connect(DB_USUARIOS)
    c = conn.cursor()
    c.execute("SELECT ultima_solicitud, solicitudes_realizadas FROM usuarios WHERE id = ?", (user_id,))
    resultado = c.fetchone()
    conn.close()
    ...
    ultima_solicitud, solicitudes_realizadas = resultado
    limites = obtener_limites_usuario(user_id)
...
def registrar_usuario(user_id: int, plan: str = "free") -> bool:
    conn = sqlite3.connect(DB_USUARIOS)
    c = conn.cursor()
    ...
    if c.fetchone():
        # Usuario existe, actualizar plan y fecha de expiración
        c.execute("UPDATE usuarios SET plan = ?, fecha_expiracion = ? WHERE id = ?", 
                 (plan.lower(), fecha_expiracion, user_id))
        conn.commit()
        conn.close()
        return True
    else:
        # Nuevo usuario
        c.execute("INSERT INTO usuarios (id, plan, fecha_registro, fecha_expiracion) VALUES (?, ?, ?, ?)", 
                 (user_id, plan.lower(), int(time.time()), fecha_expiracion))
        conn.commit()
        conn.close()
        return True
...
def restaurar_plan_usuario(user_id: int) -> bool:
    ...
    if plan != "free":
        ...
        fecha_expiracion = int(time.time()) + (duracion_dias * 24 * 3600)
        c.execute("UPDATE usuarios SET fecha_expiracion = ?, ultima_solicitud = 0, solicitudes_realizadas = 0 WHERE id = ?", 
                 (fecha_expiracion, user_id))
    else:
        # Para plan free, solo reiniciar contadores
        c.execute("UPDATE usuarios SET ultima_solicitud = 0, solicitudes_realizadas = 0 WHERE id = ?", (user_id,))
...
async def bin_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not puede_realizar_solicitud(user_id):
        plan = obtener_plan_usuario(user_id)
        if plan == "free":
            await update.message.reply_text("🚫 Has excedido tu límite de 3 solicitudes por hora (Plan Free).")
        else:
            limites = obtener_limites_usuario(user_id)
    ...
    if resultados:
        ...
        if len(resultados) == max_tarjetas:
            plan = obtener_plan_usuario(user_id)
            if plan == "free":
                respuesta += f"\n\nℹ️ Límite de {max_tarjetas} tarjetas alcanzado (Plan Free)."
            else:
...
async def start_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not usuario_autorizado(user_id):
        ...
        registrar_usuario(user_id, "free")
...
async def register_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if registrar_usuario(target_id, plan):
        if plan == "free":
...
async def users_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = sqlite3.connect(DB_USUARIOS)
    c = conn.cursor()
    c.execute("SELECT id, plan, fecha_expiracion, solicitudes_realizadas FROM usuarios")
    usuarios = c.fetchall()
    conn.close()
    ...
    for usuario in usuarios:
        user_id, plan, fecha_expiracion, solicitudes_realizadas = usuario
